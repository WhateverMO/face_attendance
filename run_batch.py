...
def main():
    ...
    targets = [video_dir / a for a in sys.argv[1:] if (video_dir/a).exists()] if len(sys.argv) > 1 else sorted(list(video_dir.glob("*.mp4")))
    ...
    if csv_path.exists():
        ...
        report = engine.db.select(["id", "name"]).join(old.select(["id"] + v_cols), on="id", how="left").fill_null(0)
    else:
    ...
    for v_p in targets:
        ...
        if v_p.name in report.columns: report = report.drop(v_p.name)
        report = report.with_columns([pl.Series(name=v_p.name, values=[hits[s] for s in engine.ids])])
    ...
    engine.save_db()
    final = report.select(["id", "name"] + v_cols + ["Total_Hits", "Attendance_Count", "Absence_Count"]).sort("id")
    final.write_csv(csv_path)

    print("\n" + "="*70)
    with pl.Config(tbl_rows=-1, tbl_cols=-1): print(final)
    print("="*70)
